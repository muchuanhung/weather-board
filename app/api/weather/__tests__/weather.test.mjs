...
test('台／臺 aliases, official counties, English spacing, and invalid object keys', () => {
  ...
  for (const county of OFFICIAL_COUNTIES)
    assert.equal(resolveCity(county.replaceAll('臺', '台')).countyName, county)
  assert.equal(resolveCity(' NEW TAIPEI ').countyName, '新北市')
  for (const invalid of ['XXX', '__proto__', 'constructor', 'toString'])
    assert.equal(resolveCity(invalid), null)
})

test('evening forecast never merges tomorrow daytime into today; elements align by date', () => {
  const weekly = location(
    element('最高溫度', 'MaxTemperature', [
      [time(22, '18'), 26],
      [time(23, '06'), 34],
      [time(23, '18'), 27],
    ]),
    element('最低溫度', 'MinTemperature', [
      [time(23, '18'), 23],
      [time(22, '18'), 22],
      [time(23, '06'), 28],
    ]),
...
test('calendar labels use Taiwan date even with a UTC clock and skip past dates', () => {
  ...
  assert.deepEqual(
    buildDailyForecast(weekly, 7, '2026-09-22T16:30:00Z').map((x) => x.day),
    ['今天', '明天']
  )
...
test('missing numbers stay null while zero and legitimate negative temperatures survive', () => {
  ...
  const hourly = location(
    element(
      '溫度',
      'Temperature',
      values.map((value) => [time(22, '12'), value])
    )
  )
  assert.deepEqual(
    buildHourlyForecast(hourly, 9).map((x) => x.temperature),
...
test('current weather uses forecast fallback and keeps optional metrics null', () => {
  const current = buildCurrentWeather({
    observation: null,
    sun: null,
  ...
  for (const field of ['windSpeed', 'pressure', 'uvIndex', 'sunrise', 'sunset'])
    assert.equal(current[field], null)
...
test('GET contract: default city, aliases, optional failures, empty forecasts, and safe errors', async (t) => {
  ...
  t.mock.method(globalThis, 'fetch', async (input) => {
    ...
    return Response.json({
      success: 'true',
      records: { Locations: [{ Location: mode === 'empty' ? [] : [forecast] }] },
    })
  })
